      ...
      <form class="xl:w-4/12 lg:w-6/12 md:6/12 w-11/12 mx-auto py-2 my-4">
        ...
        <section class="w-10/12 flex flex-wrap justify-center items-center lg:py-6 py-4 mx-auto">
          <section class="w-6/12 text-left">
            <button :disabled="loadingAll" v-on:click="goToUsersSearch()" class="w-11/12 mx-auto rounded-sm inline-block text-sm font-normal text-white bg-blue-800 py-2" type=button >
              Ver Todos
            </button>
          </section>
          <section class="w-6/12 text-right">
            <button :disabled="loadingUser" v-on:click="getUser()" class="w-11/12 mx-auto rounded-sm inline-block text-sm font-normal text-white bg-green-400 py-2" type=button >
              <svg v-if="loadingUser" class="animate-spin inline mx-1 w-1/12 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
